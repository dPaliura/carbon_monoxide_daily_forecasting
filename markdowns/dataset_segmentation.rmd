...
```latitude, longitude```. To see if so I will create a subtable with
...
And last thing - set new values into table:

```{r data-set change with provided fixes}
...
```{r check key for providing a uniqueness}
sites <- DT %>% 
...
All code will be moved into script. Current R Markdown does not write new files.
